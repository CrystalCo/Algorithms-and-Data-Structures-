...
public class A3TwoThreeTreeBSTPut<K extends Comparable<? super K>, V> {
	...
	private Node put(Node x, K key, V val) {
		...
		int cmp = key.compareTo(x.keys.get(0));
		if      (cmp < 0)
			x.children.set(0, put(x.children.get(0),  key, val));
		else if (cmp > 0)
			x.children.set(1, put(x.children.get(1), key, val));
		else
			x.values.set(0, val);
		return x;
	}
